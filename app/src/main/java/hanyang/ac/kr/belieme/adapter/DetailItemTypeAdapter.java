...
public class DetailItemTypeAdapter extends RecyclerView.Adapter {
    private Context context;
    private List<Item> itemList;
    private boolean isAdminMode;

    public DetailItemTypeAdapter(Context context) {
        this.context = context;
        itemList = new ArrayList<>();
        isAdminMode = true;
    }

    public DetailItemTypeAdapter(Context context, boolean isAdminMode) {
        this.context = context;
        itemList = new ArrayList<>();
        this.isAdminMode = isAdminMode;
    }

    public DetailItemTypeAdapter(Context context, List<Item> itemList) {
        this.context = context;
                    ...
                    switch (lastHistory.getStatus()) {
                        case USING:
                            if(isAdminMode) {
                                ...
                                formatter = new SimpleDateFormat("MM월 dd일");
                                formatter.setTimeZone(timeZone);
                                contentString = formatter.format(lastHistory.getResponseTimeStamp()) + "부터 대여 중 입니다.";
                            }
                            break;
                        case DELAYED:
                            if(isAdminMode) {
                        ...
                        case REQUESTED:
                            if(isAdminMode) {
                                ...
                                formatter.setTimeZone(timeZone);
                                contentString = formatter.format(lastHistory.getRequestTimeStamp()) + "에 먼저 요청이 들어왔습니다.";
                            }
                            break;
                        case EXPIRED:
                        case RETURNED:
